...
class ClassAbilities(AdventureMixin):
    ...
    async def insight(self, ctx: commands.Context):
        ...
        if c.hc is not HeroClasses.psychic:
            ...
            min_cooldown_time = 300 if self._sessions[SESSION_ID].easy_mode else 240
            ...
            if c.heroclass["cooldown"] <= time.time():
                ...
                if good:
                    ...
                    if session.no_monster:
                        ...
                        msg += _(f"Your third eye reveals this monster's weakness. {self.emojis.dice}({int(round(roll * max_roll))})\n")
                        ...
                        revealed_stats = []
                        if roll >= 0.98:
                            revealed_stats = ["physical", "magic", "diplomacy"]
                        elif roll >= 0.95:
                            revealed_stats = random.sample(["physical", "magic", "diplomacy"], random.choice([1, 2]))
                        if "physical" in revealed_stats:
                            real_pdef = max(pdef, 0.5)
                            msg += _("Blades will cut **{pdef}x as effectively** against this monster.\n").format(
                                pdef=round(1 / real_pdef, 2),
                            )
                        elif roll >= physical_roll:
                            if pdef >= 2.0:
